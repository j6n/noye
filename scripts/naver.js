...
var music = function(id, msg) {
  ...
  _.forEach(results, function(r) {
    ...
    var res = http.get("http://music.naver.com/video/popupPlayerVideo.nhn?videoId=" + r.Id, {"Referer": "http://music.naver.com"})
    var matches = mvpRegex.exec(res.body)
    ...
    res = http.get(url, {"Referer": "http://serviceapi.rmcnmv.naver.com/flash/shareServicePlayer.nhn", "User-Agent": mobileUA})
    var j = JSON.parse(res.body)
    var sorted = _.max(j.videoList, function(vid) { return parseFloat(vid.videoBitrate) })

    res = http.follow(sorted.playUrl)
    res = http.shorten(res.body)
    msg.Reply("[%s] %s | %s", sorted.encodingOptionName, j.postSubject, res.body)
  })
}
